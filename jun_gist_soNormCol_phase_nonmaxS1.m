...
function F = jun_gist_soNormCol_phase_nonmaxS1(mode)
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_s1NormCol_phase_nonmaxS1_%i_orient_%i_scene.mat', ...
           length(rot), n));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = S1_soNorm_phase_nonmaxS1(output, filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_s1NormCol_phase_nonmaxS1_%i_orient_%i_scene.mat', ...
            length(rot),n)) ,'F','-v7.3');
    end
end
