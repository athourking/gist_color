...
for n = 1:Nscenes
    ...
    [pyr,pind] = buildSpyr_color(img,4-imSubSample,'sp5Filters'); %4 levels (scales), 6 orientations
    ...
    oldSiz = 1;
    for ss = 1:size(pind{jj},1) %orientation*scale+2
    
        for jj =  1:numChannel
            siz = pind{jj}(ss,:);
            tmpG{n}{ss}(:,:,jj) = reshape(pyr{jj}(oldSiz:(siz(1)*siz(2)+oldSiz-1),:),[siz(1),siz(2)]);           
        end
        
        oldSiz = oldSiz + siz(1)*siz(2);
        y(:,:,:,ss) = downN(tmpG{n}{ss}, w);
...
for c = 1:Nclasses
    ...
    netc = svmtrain(netc, g(train,:), 2*(C(train)==c)-1, [], 1);
    
    [Y, scores(c,:)] = svmfwd(netc, g(test,:)); %��c���Ӧ�Ĳ��Լ�Ԥ�����ͷ���
