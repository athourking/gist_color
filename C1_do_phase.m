function g = C1_do_phase(stim, filter,filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases)
        ...
        for iPhase = 1:numPhases
            ...
            s1{iPhase}{iBand}{iScale} = single_opponents_S1(stim,filters{sc}{iPhase},numChannel,numSimpleFilters);%double opponent simple cell
             if(~INCLUDEBORDERS)
                 ...
                 s1{iPhase}{iBand}{iScale}(:,:,jj,ii) = removeborders(s1{iPhase}{iBand}{iScale}(:,:,jj,ii),fSiz(iUFilterIndex+32*(sc-1)));
...
%         ss1{iBand}{iScale} = sqrt(s1{1}{iBand}{iScale}.^2 + s1{2}{iBand}{iScale}.^2);
         ...
         for iFilt = 1:numSimpleFilters %
            for jj = 1:numChannel/2
                 for iPhase = 1:numPhases
                 sss1{iBand}{iScale}(:,:,jj,iFilt,iPhase) = sqrt(d{iPhase}{iBand}{iScale}(:,:,jj,iFilt).^2 + d{iPhase}{iBand}{iScale}(:,:,jj+numChannel/2,iFilt).^2);
                 end
            end
         end
...
for iBand = 1:numScaleBands
        for jj=1:numChannel/2
            for iFilt = 1:numSimpleFilters
                for iPhase = 1:numPhases
                c1{iBand}(:,:,jj,iFilt,iPhase) = zeros(size(sss1{iBand}{1}(:,:,jj,iFilt,iPhase)));
                for iScale = 1:length(ScalesInThisBand{iBand});
                    c1{iBand}(:,:,jj,iFilt,iPhase) = max(c1{iBand}(:,:,jj,iFilt,iPhase),sss1{iBand}{iScale}(:,:,jj,iFilt,iPhase));
                end
            end
...
g = gistGabor_C1_opp_phase(c1,numScaleBands,numChannel/2,numSimpleFilters,numPhases);
