function g = gaborS1_opp_maxAllCol(stim, filter,filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel)
...
for iBand = 1:numScaleBands
    for iScale = 1:length(ScalesInThisBand{iBand})
        ...
        for iPhase = 1:numPhases
            s1{iPhase} = single_opponents_S1(stim,filters{sc}{iPhase},numChannel,numSimpleFilters);%double opponent simple cell
            ...
            %
            %             end
            s1{iPhase} = im2double(s1{iPhase});
            
        end
        
        ss1{iBand}{iScale} = sqrt(s1{1}.^2 + s1{2}.^2);
          
    
%       for iFilt = 1:numSimpleFilters %
%             for jj = 1:numChannel/2
                 sss1{iBand}{iScale} = shiftdim(shiftdim(max(ss1{iBand}{iScale},[],3),3),1);
%             end
%       end
...
g = gistGabor_S1_maxCol(sss1,numScaleBands,2,numSimpleFilters);
