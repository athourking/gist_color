...
function F = jun_gist_coNormCol(mode)
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_c1NormCol_%i_orient_%i_scene.mat', ...
            length(rot),n));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    save(fullfile(outDir,sprintf('F_c1NormCol_%i_orient_%i_scene.mat', ...
           length(rot), n)) ,'F','-v7.3');
    end
end
