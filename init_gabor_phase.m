...
% modified from init_gabor(rot, RF_siz, Div) of Thomas Serre's Gabor
% implementation in HMAX
...
for pp = 1:numPhases
    ...
    for k = 1:numFilterSizes
        for r = 1:numSimpleFilters
            ...
            for i = -filtSizeL:filtSizeR
                for j = -filtSizeL:filtSizeR
                    ...
                    if ( sqrt(i^2+j^2)>filtSize/2 )
                        ...
                        E = exp(-(x^2+G^2*y^2)/(2*sigmaq))*cos(2*pi*x/lambda(k) + phase);
                    end
            ...
            filters(1:filtSize^2,p,pp)=reshape(f,filtSize^2,1);
            fSiz(p)=filtSize;
