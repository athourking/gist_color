...
[fSiz_gray,filters_gray,c1OL_gray,numSimpleFilters_gray] = init_gabor_phase(rot, RF_siz, Div,numPhases); %Thomas's gabor
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_do_%i_orients_%i_scene.mat', ...
           length(rot), n));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = C1_do(output, filter,filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_do_%i_orients_%i_scene.mat', ...
           length(rot), n)) ,'F','-v7.3');
    end
end
