...
function F = jun_gist_soNormCol(mode)
...
for n = 1:Nscenes
    disp([n Nscenes]);
%     outFile = fullfile(outDir,sprintf('F_s1NormCol_nonmaxS1_%i_orient_%i_scene.mat', ...
%            length(rot), n));
%         
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = S1_soNorm(output, filters, fSiz, c1SpaceSS, ...
...
save(fullfile(outDir,sprintf('F_s1NormCol_rg_nonmaxS1_%i_orient.mat', ...
           length(rot))) ,'F','-v7.3');
