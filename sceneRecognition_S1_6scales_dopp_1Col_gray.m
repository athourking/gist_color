...
%         img = imresize(img,[imageSize NaN]);
    ...
    imgpat = repmat(img,[1 1 3]);
    output = prefilt(imgpat, fc_prefilt);
%     g = gistGabor(output, numberBlocks, G);
%     g = gaborS1_phase(output, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numPhases);
    g = gaborS1_dopp_sumCol_norm(output, filter,filters, fSiz, c1SpaceSS, ...
...
save F_S1_6scales_dopp_1col_gray F
save perf_S1_6scales_dopp_1col_gray perf
