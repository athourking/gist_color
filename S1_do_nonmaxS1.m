function g = S1_do_nonmaxS1(stim, filter,filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases)
    ...
    for iScale = 1:length(ScalesInThisBand{iBand})
        ...
        for iPhase = 1:numPhases
            ...
            s1{iPhase}{iBand}{iScale} = single_opponents_S1(stim,filters{sc}{iPhase},numChannel,numSimpleFilters);%double opponent simple cell
             if(~INCLUDEBORDERS)
                 ...
                 s1{iPhase}{iBand}{iScale}(:,:,jj,ii) = removeborders(s1{iPhase}{iBand}{iScale}(:,:,jj,ii),fSiz(iUFilterIndex+numSimpleFilters*numChannel*(sc-1)));
      ...
      for jj=1:numChannel
       for ii = 1:numSimpleFilters
        ...
        for iPhase = 1:numPhases
          E_single(:,:,jj) = E_single(:,:,jj) + 0.25 *  normaliseminmax(s1{iPhase}{iBand}{iScale} (:,:,jj,ii)).^2;
        end
...
for ii = 1:numSimpleFilters
    for jj=1:numChannel
           s1{iPhase}{iBand}{iScale}(:,:,jj,ii) = sqrt(k * normaliseminmax((s1{iPhase}{iBand}{iScale}(:,:,jj,ii))).^2 ./ (sigma.^2 + E_single(:,:,jj)));
    end
end
   end
...
g = gistGabor_S1_opp(sss1,numScaleBands,numel(sss1{iBand}),numChannel/2,numSimpleFilters);
