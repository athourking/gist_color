...
function F = jun_gist_c1do_nonmaxS1_indoor(mode)
...
Nfeatures = length(rot)*(length(c1ScaleSS)-1)*numberBlocks^2 * numChannel/2;
% % 
...
switch mode
    ...
    for n = 1:numel(cI{ii})
    outFile = fullfile(outDir,sprintf('F_do_nonmaxS1_indoor_%i_orient_%i_scene_%i_dataset.mat', ...
           length(rot), n,ii));
    ...
    g = C1_do_nonmaxS1(output, filter,filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
   save(fullfile(outDir,sprintf('F_do_nonmaxS1_indoor_%i_orient_%i_scene_%i_dataset.mat', ...
