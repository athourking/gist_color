...
Nfeatures = length(rot)*(length(c1ScaleSS)-1)*numberBlocks^2 * numChannel/2*numPhases;
% % 
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_dophase_%i_scene.mat', ...
            n));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = C1_do_phase(output, filter,filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_dophase_%i_scene.mat', ...
            n)) ,'F','-v7.3');
    end
end
