...
function F = jun_gist_coNormCol_normS1(mode)
...
[fSiz,filter, filters,c1OL,numSimpleFilters] = init_color_gabor(rot, RF_siz, Div,numChannel,numPhases);
    case 1
[fSiz,filter, filters,c1OL,numSimpleFilters] = init_color_gabor_rg(rot, RF_siz, Div,numChannel,numPhases);
end
...
for n = 1:Nscenes
    disp([n Nscenes]);
%     outFile = fullfile(outDir,sprintf('F_c1NormOri_nonmaxS1_%i_orient_%i_scene.mat', ...
%             length(rot),n));
%         
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = C1_soNorm_nonmax(output, filters, fSiz, c1SpaceSS, ...
...
%     end
    ...
    save(fullfile(outDir,sprintf('F_c1NormCol_nonmaxS1_%i_orient_%i_chan_JPG.mat', ...
           length(rot), mode)) ,'F','-v7.3');
