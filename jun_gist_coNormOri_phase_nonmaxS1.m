...
% function F = jun_gist_coNormOri_phase_nonmaxS1(mode)
...
outDir = sprintf('/gpfs/scratch/jz7/colorGist/C1/%i_train',NtrainingPerClass);
if ~exist(outDir)
    mkdir(outDir);
end
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_c1NormOri_phase_nonmaxS1_%i_orient_%i_scene.mat', ...
            length(rot),n));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = C1_soNormOri_phase_nonmaxS1(output, filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_c1NormOri_phase_nonmaxS1_%i_orient_%i_scene.mat', ...
           length(rot), n)) ,'F','-v7.3');
    end
end
