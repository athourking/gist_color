...
function F = jun_gist_s1do_nonmaxS1_indoor_2orients(mode)
% This script trains a for classification.
%
...
outDir = sprintf('/gpfs/data/jz7/ColorGist/results/indoor/%i_train/S1',NtrainingPerClass);
if ~exist(outDir)
    mkdir(outDir);
end
...
switch mode
    ...
    outFile = fullfile(outDir,sprintf('F_do_%i_orients_%i_scene_%i_dataset.mat', ...
            length(rot),n,ii));
    ...
    g = S1_do_nonmaxS1(output, filter,filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_do_%i_orients_%i_scene_%i_dataset.mat', ...
            length(rot),n,ii)) ,'F','-v7.3');
    end
    end
% end

save(fullfile(outDir,sprintf('F_do_indoor_%i_orient_%i_dataset.mat', ...
