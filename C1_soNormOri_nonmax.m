function g = C1_soNormOri_nonmax(stim, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases)
...
for iBand = 1:numScaleBands
   ...
   for ii = 1:numSimpleFilters
      for jj=1:numChannel
        for iPhase = 1:numPhases
          E_single(:,:,jj) = E_single(:,:,jj) + 0.25 *  normaliseminmax(s1{iBand}{iScale} (:,:,jj,ii,iPhase)).^2;
        end     
      end
   end
% 
...
for ii = 1:numSimpleFilters
    for jj=1:numChannel
           s1{iBand}{iScale}(:,:,jj,ii,iPhase) = sqrt(k * normaliseminmax(s1{iBand}{iScale}(:,:,jj,ii,iPhase)).^2 ./ (sigma.^2 + E_single(:,:,jj)));
    end
end
   end
%    
...
%                 for iPhase = 1:numPhases
                c1{iBand}(:,:,jj,iFilt) = zeros(size(ss1{iBand}{1}(:,:,jj,iFilt)));
                for iScale = 1:length(ScalesInThisBand{iBand});
                    c1{iBand}(:,:,jj,iFilt) = max(c1{iBand}(:,:,jj,iFilt),ss1{iBand}{iScale}(:,:,jj,iFilt));
                end
%             end
...
g = gistGabor_C1_opp(c1,numScaleBands,numChannel,numSimpleFilters);
