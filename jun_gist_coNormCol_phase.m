...
function F = jun_gist_coNormCol_phase(mode)
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_c1NormCol_phase_%i_orient_%i_scene.mat', ...
            length(rot),n));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    g = C1_soNorm_phase(output, filters, fSiz, c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_c1NormCol_phase_%i_orient_%i_scene.mat', ...
           length(rot), n)) ,'F','-v7.3');
    end
end
