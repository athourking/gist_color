function g = C1_soNorm_nonmax(stim, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases)
...
for iBand = 1:numScaleBands
   ...
   for ii = 1:numSimpleFilters
      for jj=1:numChannel
        for iPhase = 1:numPhases
          E_single(:,:,ii) = E_single(:,:,ii) + 0.25 *  normaliseminmax(s1{iBand}{iScale} (:,:,jj,ii,iPhase)).^2;
        end     
      end
   end
% 
...
for ii = 1:numSimpleFilters
    for jj=1:numChannel
           s1{iBand}{iScale}(:,:,jj,ii,iPhase) = sqrt(k * normaliseminmax(s1{iBand}{iScale}(:,:,jj,ii,iPhase)).^2 ./ (sigma.^2 + E_single(:,:,ii)));
    end
end
   end
%    
