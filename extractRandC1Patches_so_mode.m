function cPatches = extractRandC1Patches_so_mode(cItrainingOnly, numPatchSizes, numPatchesPerSize, patchSizes, numChannel,numTrain,mode)
...
if ~exist(outDir)
 ...
 imageSize = 256;   
	...
	for j = 1:numPatchSizes
        cPatches{j} = zeros(patchSizes(j)^2*length(rot)*numChannel*numPhases,numPatchesPerSize);%4������8����ɫ�ŵ�
    end
    ...
    for i = 1:numPatchesPerSize
       ...
       if m ~= imageSize || n ~= imageSize 
		...
		[c1source,s1source] = C1_soppNorm_maxCol(stim, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases);
