function g = gaborS1(stim, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,INCLUDEBORDERS)
...
% nFilts = size(filters,2); %2�ֳ߶ȣ�ÿ���߶���4�������ܹ�8��
    ...
    for i = 1:nFilts
        sqfilter{i} = reshape(filters(1:(fSiz(i)^2),i),fSiz(i),fSiz(i)); %ÿ���߶���ÿ�������ϵ��˲���
        if USECONV2
        ...
        for iScale = 1:length(ScalesInThisBand{iBand}) %�ӵ�һ���߶ȿ�ʼѭ��
            for iFilt = 1:numSimpleFilters %�ӵ�һ������ʼѭ��
                ...
                if ~USECONV2
                    ...
                    %                 s1(:,:,iBand,iScale,iFilt) = abs(conv2(stim,sqfilter{iUFilterIndex},'same'));
%                     s1(:,:,iBand,iScale,iFilt) = abs(conv2padded(stim,sqfilter{iUFilterIndex}));
                     s1(:,:,iBand,iScale,iFilt)  = abs(conv2(stim,sqfilter{iUFilterIndex},'same'));
...
%     for iScale = 1:length(ScalesInThisBand{iBand}); %�ӵ�һ���߶ȿ�ʼѭ��
%       c1{iBand}(:,:,iFilt) = max(c1{iBand}(:,:,iFilt),s1{iBand}{iScale}{iFilt}); %ȡ��ÿ������ϴ�Ĳ�ͬ�߶��µ�s1��Ӧ
%     end
%   end
% end
%
