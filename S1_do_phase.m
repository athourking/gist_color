function g = S1_do_phase(stim, filter,filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases)
      ...
      d{iPhase}{iBand}{iScale} = double_opponents_d1(stim, s1{iPhase}{iBand}{iScale},filter{sc}{1},numChannel,numSimpleFilters);
         ...
         for iFilt = 1:numSimpleFilters %
            for jj = 1:numChannel/2
                for iPhase = 1:numPhases
                    sss1{iBand}{iScale}(:,:,jj,ii,iPhase) = sqrt(d{iPhase}{iBand}{iScale}(:,:,jj,ii).^2 + d{iPhase}{iBand}{iScale}(:,:,jj+numChannel/2,iFilt).^2);
                end
            end
         end
...
g = gistGabor_S1_opp_phase(sss1,numScaleBands,numel(sss1{iBand}),numChannel/2,numSimpleFilters,numPhases);
