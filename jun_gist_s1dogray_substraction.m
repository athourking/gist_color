...
Nfeatures = length(rot)*length(RF_siz)*numberBlocks^2 * numChannel/2;
[fSiz_gray,filters_gray,c1OL,numSimpleFilters] = init_gabor_phase(rot, RF_siz, Div,numPhases); %Thomas's gabor
Nfeatures_gray = length(rot)*(length(c1ScaleSS)-1)*numberBlocks^2;
...
for n = 1:Nscenes
    disp([n Nscenes]);
    outFile = fullfile(outDir,sprintf('F_dogray_substraction_%i_scene.mat', ...
            n));
    ...
    img_gray = mean(img,3);%����ɫͼ���Ϊ�Ҷ�ͼ�񣬵���ֵ�ձ����255��������ʾ�󲿷�Ϊ��ɫ
    if size(img,1) ~= imageSize
        img = imresize(img, [imageSize imageSize], 'bilinear');
%         img = imresize(img,[imageSize NaN]);
    end
    output_gray = prefilt(double(img_gray), fc_prefilt);
    output = prefilt(double(img), fc_prefilt);
    g = S1_dogray_substraction(output, output_gray,filter,filters, fSiz, fSiz_gray,filters_gray,c1SpaceSS, ...
        c1ScaleSS, c1OL,numChannel,numPhases);

    F(n,:) = g;
    
    save(fullfile(outDir,sprintf('F_dogray_substraction_%i_scene.mat', ...
            n)) ,'F','-v7.3');
    end
end
