...
addpath(genpath('/users/jz7/data/recent/ColorGist/Gist_NP'));
...
%         img = imresize(img,[imageSize NaN]);
    ...
    output = postreceotoral(double(img));
%      output = post(double(img));
%      output = fhpkgcolor_v1like_preprocess_rgb(img,q);
%     g = gistGabor(output, numberBlocks, G);
%     g = gaborS1_phase(output, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numPhases);
    g = gaborS1_dopp_norm(output, filter,filters, fSiz, c1SpaceSS, ...
...
save F_S1_6scales_dopp_4col_norm_postreceotoral F
save perf_S1_6scales_dopp_4col_norm_postreceotoral perf
